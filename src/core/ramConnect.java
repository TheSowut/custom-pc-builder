...
public class ramConnect {
    ...
    public void close(){
        try {
            ...
            Logger.getLogger(ramConnect.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public ArrayList<String> add(String[] columns, String manufacturer, String model, String memoryType, String memorySize, String price) {
        ArrayList<String> result = new ArrayList<String>();
        String cols = String.join(", ", columns);
        String sql = "SELECT " + cols + " FROM ram";
        String queryInsertData = "INSERT INTO ram (Manufacturer, Model, Memory_Type, Memory_Size, Price) values ('" + manufacturer + "', '" + model + "', '" + memoryType + "', '" + memorySize + "', '" + price + "')";
        try {
            Connection openConn = conn;
            Statement stmt  = openConn.createStatement();
